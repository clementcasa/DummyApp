...
class GetUserListUseCaseTests {
    ...
    private val useCase = GetUserListUseCase(mockUserRepository)
    ...
    fun testInvoke() {
        ...
        whenever(mockUserRepository.getUserList(any(), any())).thenReturn(Single.just(expectedDomainUserList))
        
        useCase.invoke(expectedPage)
                .test()
                .assertValue(expectedDomainUserList)
        
        verify(mockUserRepository).getUserList(expectedPage, expectedLimit)
    }
}